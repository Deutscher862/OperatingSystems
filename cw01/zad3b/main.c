...
void save_to_report(char* message){
        char str[100];
        sprintf(str, "echo %s >> results3b.txt", message);
        system(str);
}
...
int main(int argc, char **argv){
    ...
    //argv[3] = optimalization_level
    sprintf(message, "Optimalization_level: %s", argv[3]);
    save_to_report(message);

    //argv[4] = test_type
    sprintf(message, "Test_type: %s results:", argv[4]);\
    save_to_report(message);
    save_to_report("");
