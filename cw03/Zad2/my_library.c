...
RowBlock *create_row_block(){
    RowBlock *res = malloc(sizeof(struct RowBlock));
    return res;
}
...
MainArr *create_main_arr(int size){
    ...
    MainArr *res = malloc(sizeof(struct MainArr));
    res->size = size;
    res->current_pair = 0;
...
void save_files_into_blocks(MainArr* arr){
    char filename[100];
    for(int i = 0; i < arr->size; i++){
        snprintf(filename, sizeof(filename), "output/tmp%d.txt", i);

        int number_of_rows = count_file_lines(filename);
        RowBlock* block = arr->block_arr[i];
        
        if(block == NULL) exit(-1);

        block->size = number_of_rows;
        block->rows_arr = calloc(number_of_rows,sizeof(char *));
...
void merge_files(MainArr* arr, char** files, int current_pair){
    FILE* file1 = fopen(files[0], "r");
    FILE* file2 = fopen(files[1], "r");

    if(file1 != NULL && file2 != NULL){
        char str[100];
        sprintf(str, "paste %s %s -d '\\n' | grep -vE \"$^\" > output/tmp%d.txt",
